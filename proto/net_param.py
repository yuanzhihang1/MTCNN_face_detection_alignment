...
class Net_Prototxt():
    ...
    def layer_param(self,layer_name):
        """
    ...
    def add_layer(self, layer_name, layer_params, after='',index=-1):
        if after != '':
            for i, layer in enumerate(self.net.layer):
                if layer.name == after:
                    index = i + 1
...
class Layer_param():
    ...
    def conv_param(self,num_output,kernel_size,stride=(1),weight_filler='xavier',bias_filler='constant'):
        """
        ...
        conv_param.weight_filler.type=weight_filler
        conv_param.bias_filler.type = bias_filler
